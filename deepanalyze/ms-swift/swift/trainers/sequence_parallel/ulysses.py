...
from .utils import (
    ...
    _get_per_token_logps_grpo,
    _get_train_sampler_grpo,
    _prepare_inputs,
    _prepare_inputs_grpo,
    get_common_dataloader,
    get_per_token_logps,
    loss_scale_sp_func,
    old_policy_grpo,
    padding_free_context_grpo,
    setup_compute_acc,
...
def _generate_layout_params(scatter_idx, seq_world_size, input):
    if scatter_idx < 2:
        ...
        assert num_total_head % seq_world_size == 0, (
            f"Number of heads ({num_total_head}) must be divisible "
        ...
        **kwargs,
        ...
        if position_ids is not None:
            ...
            position_ids_output = torch.empty(
                (shape0 * dist.get_world_size(self.spg), position_ids.shape[1]),
                dtype=position_ids.dtype,
                device=position_ids.device,
            )
            dist.all_gather_into_tensor(
                position_ids_output, position_ids, group=self.spg
            )
...
class Ulysses(CommonSequenceParallel):
    ...
    def init_sequence_parallel(self, size):
        ...
        ALL_ATTENTION_FUNCTIONS["flash_attention_2_origin"] = ALL_ATTENTION_FUNCTIONS[
            "flash_attention_2"
        ]
        ALL_ATTENTION_FUNCTIONS["sdpa_origin"] = ALL_ATTENTION_FUNCTIONS["sdpa"]

        def local_flash_attn(
            ...
            **kwargs,
            ...
            if dist_attn.local_attn is None:
                ...
                def _attention(query, key, value, *args, **kwargs):
                    ...
                    key = key.transpose(1, 2)
                    value = value.transpose(1, 2)
                    return ALL_ATTENTION_FUNCTIONS["sdpa_origin"](
                        module, query, key, value, *args, **kwargs
                    )[0]
        ...
        if is_flash_attn_available():
            ...
            _distributed_flash_attention = DistributedAttention(
                _flash_attention_forward, self.sp_group
            )
    ...
    def prepare_model(self, model, tokenizer):
        ...
        def pre_forward_split_hook(_self, args, kwargs):
            ...
            _input_ids, inputs_embeds, _, position_ids, attention_mask, _ = (
    ...
    def prepare_trainer(self, trainer):
        ...
        if trainer.__class__.__name__ == "Seq2SeqTrainer":
            ...
            trainer._prepare_inputs = MethodType(
                partial(_prepare_inputs, sp_instance=self), trainer
            )
            ...
            trainer.args.gradient_accumulation_steps = (
                trainer.args.gradient_accumulation_steps * self.sp_world_size
            )
            trainer.old_policy = MethodType(
                partial(old_policy_grpo, sp_instance=self), trainer
            )
            trainer._get_train_sampler = MethodType(
                partial(_get_train_sampler_grpo, sp_instance=self), trainer
            )
            trainer._prepare_inputs = MethodType(
                partial(_prepare_inputs_grpo, sp_instance=self), trainer
            )
            trainer._get_per_token_logps = MethodType(
                partial(_get_per_token_logps_grpo, sp_instance=self), trainer
            )
            trainer.split_by_mini_batches = MethodType(
                partial(split_by_mini_batches_grpo, sp_instance=self), trainer
            )
